
// import contractDefinition from "./contractDefinition";
var contractDefinition = {
    "contract_name": "IOwnThis",
    "abi": [
      { 
        "constant": false,
        "inputs": [
          {
            "name": "claim",
            "type": "bytes32"
          }
        ],
        "name": "createClaim",
        "outputs": [],
        "payable": false,
        "type": "function"
      },
      {
        "constant": false,
        "inputs": [
          {
            "name": "claim",
            "type": "bytes32"
          }
        ],
        "name": "claimOwnership",
        "outputs": [
          {
            "name": "",
            "type": "bool"
          }
        ],
        "payable": false,
        "type": "function"
      },
      {
        "anonymous": false,
        "inputs": [
          {
            "indexed": true,
            "name": "claim",
            "type": "bytes32"
          },
          {
            "indexed": true,
            "name": "owner",
            "type": "address"
          }
        ],
        "name": "NewClaim",
        "type": "event"
      }
    ],
    "unlinked_binary": "",
    "networks": {},
    "schema_version": "0.0.5",
    "updated_at": 1507990824672
  };

export default class Client {
    
    static createHash(data) { 
        return window.web3.sha3(data, {encoding: 'hex'}); 
    }
    
    getContractInstance() {
        if(this.contractInstance) return this.contractInstance;
        var contractAddress = "0x62a2408885e7e875b447f95aaa7f92a4447d622de949892296c762a775a15cd8";
        var contract = window.web3.eth.contract(contractDefinition.abi);
        this.contractInstance = contract.at(contractAddress)
        return this.contractInstance;
    }

    sendClaim(ownData, callback) {
        var hash = Client.createHash(ownData);
        var tx = {from: window.web3.eth.defaultAccount, gas: 470000};
        this.getContractInstance().createClaim.sendTransaction("hsahs", tx, callback);
    }
}